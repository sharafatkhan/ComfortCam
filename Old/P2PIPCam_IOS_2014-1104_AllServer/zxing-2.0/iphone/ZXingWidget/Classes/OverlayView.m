...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled {
  ...
  if( self ) {
    ...
    if (isCancelEnabled) {
      ...
      [cancelButton setTitle:NSLocalizedStringFromTable(@"Cancel", @"IpCameraClient", nil) forState:UIControlStateNormal];
      if (oneDMode) {
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
    //self.displayedMessage = @"Place a barcode inside the viewfinder rectangle to scan it.";
      self.displayedMessage = NSLocalizedStringFromTable(@"ScanDisplayMessage", @"IpCameraClient", nil);
  }
